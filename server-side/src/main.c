...
BT_MESH_MODEL_PUB_DEFINE(pub_level_srv, NULL, 2 + 2 + 1);
...
static struct bt_mesh_model led1_srv_models[] = {
    BT_MESH_MODEL(BT_MESH_MODEL_ID_GEN_LEVEL_SRV, generic_level_srv_op, &pub_level_srv,
                  &led_level_elems[0]),
...
struct bt_mesh_elem elements[] = {
    ...
    BT_MESH_ELEM(0, led1_srv_models, BT_MESH_MODEL_NONE),
};
...
void set_level(u16_t pulse_width)
{
    ...
    int err;
    err = ic_leds_pwm_change_pulse(&leds_pwm_device, pulse_width);
    if (err) {
        printk("Error %d changing pulse_width\n", err);
    }
}
...
{
    int err;
    err = ic_leds_pwm_init_device(&leds_pwm_device, PWM_DRIVER, PWM_CHANNEL0, PERIOD);
    if (err) {
...
void main(void)
{
    ...
    while (1) {
        ic_leds_pwm_set_pulse(&leds_pwm_device);
        k_sleep(SLEEP_TIME);
    }
}